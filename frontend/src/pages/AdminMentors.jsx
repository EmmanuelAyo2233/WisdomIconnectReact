...
import { Search, UserCheck, ShieldAlert, X, Trash2, ArrowLeft } from 'lucide-react';
...
const AdminMentors = () => {
  ...
  const fetchMentors = async () => {
    try {
      const response = await adminService.getApprovedMentors();
  ...
  const handleReject = async (id) => {
    if(!window.confirm("Are you sure you want to reject this already approved mentor? They will be moved to mentees.")) return;
    try {
      await adminService.rejectUser(id);
      addToast("Mentor rejected successfully", "success");
      setMentors(mentors.filter(m => m.id !== id));
    } catch (e) {
      addToast("Failed to reject mentor", "error");
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Approved Mentors</h1>
          <p className="text-gray-500 text-sm mt-1">Manage active mentors on the platform.</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50/50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Approval Date</th>
              ...
              {loading ? (
                ...
                <tr key={mentor.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-xs font-medium text-gray-500">
                    {mentor.approvedDate ? new Date(mentor.approvedDate).toLocaleDateString() : 'N/A'}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button 
                      onClick={() => handleReject(mentor.id)} 
                      title="Reject Approved Mentor"
                      className="px-3 py-1.5 mr-2 rounded-lg text-xs font-medium text-red-600 hover:bg-red-50 transition-colors inline-flex items-center border border-red-100"
                    >
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500">
                    ...
                    <p className="text-lg font-medium">No approved mentors found.</p>
                  </td>
                </tr>
              )}
            </tbody>
